...
class IPAddresss:
    def __init__(self, ipdbFile):
        ...
        say = self.ipdb.read(8)
        (self.firstIndex, self.lastIndex) = struct.unpack('II', say)
    ...
    def getAreaAddr(self, offset=0):
        if offset:
            self.ipdb.seek(offset)
        say = self.ipdb.read(1)
        (byte,) = struct.unpack('B', say)
    ...
    def getLong3(self, offset=0):
        if offset:
            self.ipdb.seek(offset)
        say = self.ipdb.read(3)
        (a, b) = struct.unpack('HB', say)
        return (b << 16) + a
